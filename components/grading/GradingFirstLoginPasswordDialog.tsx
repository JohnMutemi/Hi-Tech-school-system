...
type GradingFirstLoginPasswordDialogProps = {
  schoolCode: string;
};

export function GradingFirstLoginPasswordDialog({ schoolCode }: GradingFirstLoginPasswordDialogProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      setSubmitting(true);
      const res = await fetch(`/api/schools/${schoolCode}/grading/change-password`, {
        method: "POST",
      ...
      router.replace(`/schools/${schoolCode}/grading/login?relogin=1`);
    } catch (err) {
      toast({
      ...
      <Card className="w-full max-w-md border-slate-200 shadow-2xl">
        <CardHeader className="space-y-1">
          <div
            ...
            style={{ backgroundColor: `${colorTheme}22`, color: colorTheme }}
          >
            <Lock className="h-6 w-6" />
          </div>
          <CardTitle className="text-center text-xl">Set your password</CardTitle>
          <CardDescription className="text-center">
            For security, grading-only access requires a personal password before you continue. After saving, you will
        ...
        <CardContent>
          <form onSubmit={handleSubmit} className="space-y-4">
            <div className="space-y-2">
              <Label htmlFor="grading-cur-pw">Current password</Label>
              <Input
                id="grading-cur-pw"
                type="password"
            ...
            <div className="space-y-2">
              <Label htmlFor="grading-new-pw">New password</Label>
              <Input
                id="grading-new-pw"
                type="password"
            ...
            <div className="space-y-2">
              <Label htmlFor="grading-confirm-pw">Confirm new password</Label>
              <Input
                id="grading-confirm-pw"
                type="password"
                autoComplete="new-password"
                required
            ...
            <Button type="submit" className="w-full" disabled={submitting} style={{ backgroundColor: colorTheme }}>
              {submitting ? (
                <>
