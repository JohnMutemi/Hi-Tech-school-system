...
export function SchoolSetupDashboard({
  ...
  const saveSchoolProfile = async () => {
    if (!schoolData?.schoolCode) {
      toast({ title: "Error", description: "School data not loaded.", variant: "destructive" });
      return;
    }
    await fetch(`/api/schools/${schoolData.schoolCode}/profile`, {
      method: "PUT",
  ...
  const createTeacher = async (teacherData: Partial<Teacher>) => {
    ...
    await fetch(`/api/schools/${schoolData.schoolCode}/teachers`, {
      method: "PUT",
      headers: { "Content-Type": "application/json" },
  ...
  const createStudent = async (studentData: Partial<Student>) => {
    ...
    if (
      ...
      toast({
        title: "Validation Error",
        description:
          "Please fill in required fields (Student Name, Parent Name, and Parent Phone)",
        variant: "destructive",
      });
      return false;
    }
    ...
    const student: Student = {
      ...
      dateAdmitted:
        studentData.dateAdmitted || new Date().toISOString().split("T")[0],
      status: "active",
    };

    const updatedStudents = [...students, student];
    setStudents(updatedStudents);
    await fetch(`/api/schools/${schoolData.schoolCode}/students`, {
      method: "PUT",
      headers: { "Content-Type": "application/json" },
  ...
  const createSubject = async () => {
    ...
    await fetch(`/api/schools/${schoolData.schoolCode}/subjects`, {
      method: "PUT",
      headers: { "Content-Type": "application/json" },
  ...
  const createClass = async () => {
    ...
    await fetch(`/api/schools/${schoolData.schoolCode}/classes`, {
      method: "PUT",
      headers: { "Content-Type": "application/json" },
                  ...
                  <CardHeader>
                    <CardTitle className="flex items-center justify-between">
                      <div className="flex items-center space-x-3">
                        ...
                        <span
                          className={step.completed ? "text-green-700" : ""}
                        >
                          {step.title}
                        </span>
                      </div>
                ...
                <CardHeader>
                  <CardTitle className="flex items-center justify-between">
                    ...
                    <Button
                      ...
                      style={{ backgroundColor: schoolData.colorTheme }}
                      disabled={!schoolData?.schoolCode}
                    >
                ...
                <CardContent>
                  {students.length === 0 ? (
                    <div className="text-center py-8">
                      ...
                      <p className="text-gray-600">
                        No students added yet. Click "Add Student" to get
                        started.
                      </p>
                    </div>
                  ) : (
