...
import heroPupils from "@/assets/hero-pupils-2.jpg";
import heroClassroom from "@/assets/hero-classroom-3.jpg";
import heroSports from "@/assets/hero-sports-4.jpg";
import campusImg from "@/assets/gallery-campus.jpg";

const heroSlides = [
  { src: heroImg, alt: "Pupils of Mwingi Royal Junior Academy outside the school building" },
  { src: heroPupils, alt: "Smiling pupils in uniform in front of the school" },
  { src: heroClassroom, alt: "Pupils raising hands in a bright classroom" },
  { src: heroSports, alt: "Pupils playing football on the school field" },
];
...
export const Route = createFileRoute("/")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Mwingi Royal Junior Academy is a leading primary school 8KM off Mwingi–Tseikuru road, nurturing excellence since 2016." },
      { property: "og:title", content: "Mwingi Royal Junior Academy" },
      { property: "og:description", content: "A centre of excellence nurturing curious minds since 2016." },
    ],
  }),
  component: Index,
});
...
function Index() {
  return (
    <SiteLayout>
      {/* Split-screen Hero — mobile: stacked with image on top */}
      <section className="grid grid-cols-1 lg:min-h-[88vh] lg:grid-cols-2">
        <div className="relative order-1 h-[56vh] min-h-[320px] overflow-hidden lg:order-2 lg:h-auto lg:min-h-[500px]">
          <ImageCarousel slides={heroSlides} intervalMs={5000} eager />
        </div>
      ...
      <section className="mx-auto max-w-7xl px-5 py-14 sm:px-6 sm:py-20">
        <div className="grid gap-4 sm:gap-6 md:grid-cols-3">
          {[
            { icon: BookOpen, title: "Academic Excellence", body: "Rigorous CBC-aligned learning with personal attention in every class." },
            { icon: Users, title: "Caring Community", body: "Supportive teachers, engaged parents, and lifelong friendships." },
            { icon: Trophy, title: "Co-curricular Strength", body: "Sports, arts, music, and clubs that build well-rounded children." },
          ].map(({ icon: Icon, title, body }) => (
            <div key={title} className="rounded-xl border border-border bg-card p-7 transition hover:shadow-lg">
      ...
      <section className="bg-secondary/50">
        <div className="mx-auto grid max-w-7xl items-center gap-8 px-5 py-14 sm:gap-12 sm:px-6 sm:py-20 lg:grid-cols-2">
          <img src={campusImg} alt="School campus" width={1200} height={900} loading="lazy" className="aspect-[4/3] w-full rounded-xl object-cover shadow-xl" />
          <div>
            <div className="mb-3 text-[11px] font-semibold uppercase tracking-[0.25em] text-teal">About the Academy</div>
            <h2 className="font-serif text-2xl font-bold sm:text-3xl md:text-4xl">Excellence rooted in our community.</h2>
            <p className="mt-4 text-sm leading-relaxed text-muted-foreground sm:mt-5 sm:text-base">
              Founded in 2016, Mwingi Royal Junior Academy has grown into a trusted home of learning serving families across Mwingi and beyond. Located 8KM off the Mwingi–Tseikuru road before Kyulungwa centre market, our campus offers a calm, safe, and inspiring environment for every learner to thrive.
            </p>
            <Link to="/about" className="mt-5 inline-flex items-center gap-2 text-sm font-semibold text-ocean hover:text-teal sm:mt-7">
      ...
      <section className="mx-auto max-w-7xl px-5 py-14 text-center sm:px-6 sm:py-20">
        <h2 className="font-serif text-2xl font-bold sm:text-3xl md:text-4xl">Begin your child's journey with us.</h2>
        <p className="mx-auto mt-3 max-w-2xl text-sm text-muted-foreground sm:mt-4 sm:text-base">
          Admissions are open. Download the form, complete it, and visit our school office to submit.
        </p>
        <Link
          to="/admissions"
