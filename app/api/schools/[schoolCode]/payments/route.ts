...
export async function POST(request: NextRequest, { params }: { params: { schoolCode: string } }) {
  try {
    ...
    try {
      ...
      // Notify the school's registered email using the exact same unified receipt template.
      if (schoolRecord.email) {
        schoolEmailNotificationSent = await emailService.sendPaymentNotificationToEmail(
          payment.id,
          schoolRecord.code,
          schoolRecord.email
        );
