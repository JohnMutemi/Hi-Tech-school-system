...
import { LogOut, User, BookOpen, Users, Settings, Key, Menu } from "lucide-react"
    ...
    <Card>
      ...
      <CardContent>
        <form onSubmit={handleSaveProfile} className="space-y-4">
          <input name="name" value={editData.name || ''} onChange={handleEditChange} placeholder="Name" className="w-full p-2 border rounded" />
          <input name="phone" value={editData.phone || ''} onChange={handleEditChange} placeholder="Phone" className="w-full p-2 border rounded" />
          <input name="qualification" value={editData.qualification || ''} onChange={handleEditChange} placeholder="Qualification" className="w-full p-2 border rounded" />
        ...
        {teacher?.classes?.length > 0 ? (
          <ul className="space-y-2">
            {teacher.classes.map((c: any) => (
              <li key={c.id} className="p-2 border rounded">{c.name}</li>
            ))}
          </ul>
      ...
      <aside className={`fixed inset-y-0 left-0 z-20 w-64 bg-white border-r p-4 transform transition-transform md:relative md:translate-x-0 ${isSidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        <div className="flex flex-col items-center text-center p-4 border-b">
          <Avatar className="w-24 h-24 mb-4 relative group">
            <img src={teacher?.avatarUrl || "/placeholder-user.jpg"} alt={teacher?.name} />
