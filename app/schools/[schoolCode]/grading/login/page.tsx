...
export default function GradingLoginPage() {
  ...
  const [schoolName, setSchoolName] = useState('Grading Module Access');
  const [schoolLogoUrl, setSchoolLogoUrl] = useState<string | undefined>();
  const [packageType, setPackageType] = useState('grading_only');
  ...
  React.useEffect(() => {
    ...
    if (qs.get('relogin') === '1') {
      ...
      router.replace(`/schools/${schoolCode}/grading/login`, { scroll: false });
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      setLoading(true);
      const response = await fetch(`/api/schools/${schoolCode}/grading/login`, {
        method: 'POST',
      ...
      if (response.ok) {
        const data = await response.json();
        toast({ title: 'Login Successful', description: `Welcome, ${data.user?.name || 'Teacher'}!` });
        router.push(`/schools/${schoolCode}/grading`);
      } else {
  ...
  const handleForgotPassword = async () => {
    ...
    if (!formData.email) {
      toast({
        ...
        description: 'Enter your registered grading email first.',
        variant: 'destructive',
      });
      return;
    }
    ...
    try {
      ...
      const response = await fetch(`/api/schools/${schoolCode}/grading/forgot-password`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  return (
    <SchoolLoginShell
      ...
      heading={schoolName}
      subheading={`Sign in to your ${ACADEMICS_PORTAL_LABEL}`}
      ...
      sisterModuleLink={
        isDualModulePackage(packageType)
          ? {
              label: `Open ${FINANCE_PORTAL_LABEL}`,
              href: financePortalLoginPath(schoolCode),
            }
          : undefined
      }
    >
      <form onSubmit={handleSubmit} className="space-y-5">
        ...
        {forgotInfo.message ? (
          <div className="rounded-md border border-blue-200 bg-blue-50 p-3 text-sm text-blue-700">
            ...
            {forgotInfo.link ? (
              <a href={forgotInfo.link} className="mt-2 inline-block text-blue-800 underline">
